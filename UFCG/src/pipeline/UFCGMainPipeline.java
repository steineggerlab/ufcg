...
//			+ ANSIHandler.wrapper("by LEB, SNU", 'B')
	...
	private static void printManual() {
		System.out.println(
				...
				"    Interactive mode : java -jar /path/to/UFCG.jar -u\n"+
				"    One-liner mode   : java -jar /path/to/UFCG.jar -i <PATH> -o <PATH> [options]\n\n"+
				
				ANSIHandler.wrapper("[Dependencies]\n\n", 'y') +
				ANSIHandler.wrapper("* AUGUSTUS (fastBlockSearch, AUGUSTUS-PPX)\n", 'c') +
				"    UFCG extracts genes using AUGUSTUS gene prediction tool.\n" +
...
/*				ANSIHandler.wrapper(
	...
	private static int interactiveRoute(String arg) throws IOException {
		...
		Prompt.talk("Verbose option check.");
		if(GenericConfig.TSTAMP) Prompt.talk("Timestamp printing option check.");
		Prompt.print(ANSIHandler.wrapper("Welcome to UFCG user interactive mode.", 'Y'));
...
/*		if(flag) {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Prompt.talk("Launching UFCG pipeline...\n");
			...
			for(int q = 0; q < queries.size(); q++) {
				...
				for(int g = 0; g < GenericConfig.FCG.length; g++) {
					CreateProfile creator = new UFCGMainPipeline().new CreateProfile(g);
